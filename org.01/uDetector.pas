...
begin
  ...
  a:=15;b:=0.1;theta:=3*pi/notch;
  Cen2.new(0,0,0);

  sph.add(SphereClass.Create(10000,  Cen+p.new(0,0,-200)  , e.new(0.6, 0.5, 0.7)*0.8, c.new(0.7,0.9,1.0),  DIFF)); // sky
  sph.add(SphereClass.Create(100000, p.new(50, 0,-100000), ZeroVec,                  c.new(0.4,0.4,0.4),  DIFF)); // grnd

  for n:=0 to notch do begin
     theta:=pi/6*(3+n);
     r:=a*Exp(b*theta);
     cen1:=cen2+cen1.new(r*cos(theta),r*sin(theta),5 );
